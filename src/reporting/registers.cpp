...
ctest::report::registers(const struct user_regs_struct& regs)
{
	...
	auto pair = [&](std::string_view a_name,
	                ...
	                uintptr_t b) {
		return format(
		  "{c_green}{0}{c_reset}={c_blue}{1: "
		  "<{4}x}{c_green}{2}{c_reset}={c_blue}{3: <{4}x}{c_reset}\n",
		  a_name,
		  a,
		  b_name,
		  b,
	...
	std::cerr << pair("RAX", regs.rax, "R8 ", regs.r8)
	          ...
	          << format("{c_green}RIP{c_reset}={c_blue}{0: "
	                    "<{1}x}{c_reset}\n",
	                    regs.rip,
	                    width);
	...
	std::cerr << format("{c_green}EF{c_reset}={c_blue}{0:x} {c_yellow}[ {1} ]{c_reset}\n", regs.eflags, fmt::join(flags, " "));
}
